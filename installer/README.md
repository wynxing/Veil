...
- 默认写入 `%ProgramFiles%\Veil`。首次安装可在「选项」中选择本机固定磁盘上受保护的目录；普通用户可写的目录、移动盘和网络路径会被拒绝。升级沿用原目录，迁移位置须先卸载再安装。
- 始终创建开始菜单入口。可在「选项」中勾选创建公共桌面快捷方式，默认不勾选；卸载时由 MSI 移除安装器创建的入口。
- EXE、安装器、窗口、托盘及快捷方式使用统一的屏幕与帷幕图标，托盘保持关闭时使用暖色变体。
- 安装目录、桌面快捷方式和辅助设备选择记录在 64 位 HKLM，后续升级沿用上次选择。
...
产品把 INF/DLL 放到实际安装目录的 `vdd` 子目录，并把 `vdd_settings.xml` 同时写到该目录与 **`C:\VirtualDisplayDriver`**。对捆绑 `MttVDD.dll` 的只读字符串检查显示驱动写死后者。哈希与发布者指纹见 [payload.manifest.json](payload.manifest.json)。再分发许可见仓库根目录 `NOTICE`。
...
- 恢复检查只支持可确认的当前交互用户。SYSTEM 安装动作先确认只有一个登录用户，再通过该会话主令牌和用户环境启动实际安装目录中的 App 恢复入口。其它用户仍登录（含断开的会话）、无登录用户或令牌/会话查询失败均阻止卸载；先在各用户会话恢复显示并注销其它用户。
