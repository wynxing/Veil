...
物理屏列表、按屏保持关闭与单独恢复、恢复全部、紧急热键、按需隐藏虚拟输出。虚拟屏不对用户提供开关。跨重启不自动再关。睡眠或待机中断后回放关屏前显示并打开面板；睡醒自动再关不是本轮要求，也尚未机旁验证。这些已在 `src/` 落地并有离线测试。未测 GPU、Windows 10 与 ARM 不得写入支持列表。
