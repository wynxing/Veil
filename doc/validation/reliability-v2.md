...
日期：2026-09-20。状态：代码与离线验证；本轮没有运行真实关屏、安装或卸载。不继承此前 Rust 短时记录为本版本验收。产品合同已改为「辅助输出可随应用或从面板安装、已有同一 MTT 则接管」，见 [安装计划](../plans/2026-09-20-vdd-install.md)；下列离线条目仍描述改代码前的行为。
...
覆盖：失败请求多轮去重；首次/追加/单屏恢复/克隆/醒后再关失败；恢复失败后热键和新请求重试；父进程退出；恢复专用入口；延迟枚举、拔出和布局差异；部分 APPLY 失败；损坏/旧协议结果；原子写入锁冲突；恢复超时；VDD 所有结束原因的最后输出保护；预先活动 VDD 当时不自动认领（合同已改为接管，测试待改）；清理失败不循环提权；维护状态的保守门禁。
...
| P15 | 双物理分别关闭/恢复仍可不装辅助输出；关光全部物理屏时按新合同安装或接管。面板、循环、睡醒未做 |
