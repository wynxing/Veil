...
日期：2026-09-18；状态：短时闭环、父进程崩溃恢复、10 分钟、20 次循环、虚拟目标消失、最小应用两次关屏恢复，以及关屏中睡眠唤醒已记录。唤醒后内屏亮起，不自动再关；文案修正后操作者确认界面正常。**不能写成不依赖虚拟屏，也还没有安装包。**
...
- 最小应用按该路线实现，见仓库 `app/`。本机操作者已完成两次短时保持关闭并用热键恢复；关屏中睡眠后画面正常亮起。第一次界面显示未知，文案修正后复测通过。扩展桌面与会话目录冲突导致的失败已记录并修复。不是安装包。
