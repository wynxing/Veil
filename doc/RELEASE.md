...
改版本只改 props，不要在 WiX 里手写另一套数字。`src/Cargo.toml` 的 `version` 与 `workspace.metadata.veil.suffix` 必须和这份 props 一致，发布预检会核对，不一致则失败。应用内更新比较的是 props 编出来的信息版本（例如 `0.1.12-preview.1`），不是丢掉后缀的文件版本 `0.1.12.0`。
