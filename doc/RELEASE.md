...
当前预览版本是 `0.1.11-preview.1`（`v0.1.11-preview.1`）。睡眠或待机结束后回放关屏前拓扑、结束会话且不再自动关屏，并打开面板。执行间隙与睡眠对齐；热插拔仍允许一次重新关屏。睡眠回路未经本机实测。关面板退回托盘；托盘再开面板时拉回当前工作区。第二实例会唤醒已有面板。关最后一块物理屏时可从面板安装或接管辅助 MTT。0.1.7 起升级先 `retire-old`。再改代码：升高 `Version`、合并、再打新 tag。
