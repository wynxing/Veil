...
改版本只改 props，不要在 WiX 里手写另一套数字。`src/Cargo.toml` 的 `version` 与 `workspace.metadata.veil.suffix` 必须和这份 props 一致，发布预检会核对，不一致则失败。应用内更新比较的是 props 编出来的信息版本（例如 `0.1.15-preview.1`），不是丢掉后缀的文件版本 `0.1.15.0`。
...
当前预览版本是 `0.1.15-preview.1`（`v0.1.15-preview.1`）。数字版本从 `0.1.14` 升高，已安装的预览可以升级。关第二块屏时，启用辅助输出所花的时间不再被当成会话中断。会话文件没写完时不再启动恢复进程。慢路径还没有在这台机器上重新确认。睡眠回路与唤醒闪屏的机旁矩阵仍未通过。0.1.7 起升级先 `retire-old`。再改代码：升高 `Version`、合并、再打新 tag。
