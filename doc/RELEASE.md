...
改版本只改 props，不要在 WiX 里手写另一套数字。`src/Cargo.toml` 的 `version` 与 `workspace.metadata.veil.suffix` 必须和这份 props 一致，发布预检会核对，不一致则失败。应用内更新比较的是 props 编出来的信息版本（例如 `0.1.14-preview.1`），不是丢掉后缀的文件版本 `0.1.14.0`。
...
当前预览版本是 `0.1.14-preview.1`（`v0.1.14-preview.1`）。数字版本从 `0.1.13` 升高，已安装的预览可以升级。这一版让两块物理屏可以一起保持关闭：关到最后一块时启用本机辅助虚拟输出，并按当前活动路径补上模式后再停掉剩余物理屏。本机内置屏加外接屏已确认可以同时保持关闭；其它机器未测。睡眠回路与唤醒闪屏的机旁矩阵仍未通过。0.1.7 起升级先 `retire-old`。再改代码：升高 `Version`、合并、再打新 tag。
