...
版本：1.3  
状态：实现栈已锁定；`src/` 与 `installer/` 已创建；P15 C# 短时只停内屏与热键恢复已观察；REDMI 安装器 VDD 短时有系统检查，操作者确认内屏灭了；公开产品未发布  
日期：2026-09-19
...
产品引擎**不要**把最小应用里「只关内屏」写死。目标集合是用户选中的物理路径列表。克隆调整只留给「物理路径将变为 0、必须靠自带 VDD」这一支。双物理屏扩展桌面已通过的，禁止无故改克隆。
...
| `result.json`           | Recovery    | 结束原因与恢复结果                       |


结束原因需能区分：`hotkey`、`release`、`parent-exit`、`execution-gap`（调度间隙，常见于睡眠）、`unexpected-topology`、`error`。

创建恢复进程时使用 `CREATE_BREAKAWAY_FROM_JOB | CREATE_NEW_PROCESS_GROUP | CREATE_NO_WINDOW`。父进程崩溃不得带走恢复进程。恢复进程自身崩溃不在保证范围内；人工兜底仍是 `Win+Ctrl+Shift+B`，再不行重启。
...
具体文件哈希以安装时捆绑的版本为准，写入安装器校验表；更换上游包必须重新核签名与指纹，并在验证文档记一笔。当前调研包指纹见该脚本，不把脚本路径写成产品依赖。
...
REDMI 上产品安装器路径已有一次短时闭环：装完禁用、面板 enable、只停内屏（系统检查 + 操作者确认内屏灭）、`release` 后 disable。恢复闪屏口头未做，见 [redmi-book-14-2025-csharp.md](validation/redmi-book-14-2025-csharp.md)。启用/禁用是否处处不需重启、睡眠后设备是否仍在，仍未验证。不可公开安装，也不得写成全平台可用。

## 8. 仓库布局与现存代码处置

产品合同、探针与两台机器的证据已在 main（由 verify/p15-physical-aux 合入）。公开产品实现另开分支，不要在 verify 验证分支上继续写产品。
...
安装包签名、驱动同意文案、卸载恢复，均属发布门禁；未做不得标「可公开安装」。
