...
python probe.py disable-path --config out/topology.json --watchdog-seconds 15 --confirm off --target internal --log out/disable-path.jsonl
python probe.py disable-path --config out/topology.json --confirm off --target internal --validate-only --log out/validate.jsonl
```
